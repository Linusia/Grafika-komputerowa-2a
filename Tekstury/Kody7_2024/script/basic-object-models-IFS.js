...
function uvPyramid16(radius, height, sides, noBottom) {
   radius = radius || 0.5;
   ...
   for (var i = 0; i <= sides; i++) {
      ...
      baseVerts.push([x, -height/2, z]);
   }
   for (var i = 0; i < sides; i++) {
      var p1 = baseVerts[i], p2 = baseVerts[i+1];
      var ux = apex[0]-p1[0], uy = apex[1]-p1[1], uz = apex[2]-p1[2];
      var vx = p2[0]-p1[0], vy = p2[1]-p1[1], vz = p2[2]-p1[2];
      var nx = uy*vz - uz*vy, ny = uz*vx - ux*vz, nz = ux*vy - uy*vx;
      var len = Math.hypot(nx,ny,nz);
      if (len) { nx/=len; ny/=len; nz/=len; }
      vertices.push(apex[0],apex[1],apex[2]); normals.push(nx,ny,nz); texCoords.push(0.5,1);
      vertices.push(p1[0],p1[1],p1[2]); normals.push(nx,ny,nz); texCoords.push(0,0);
      vertices.push(p2[0],p2[1],p2[2]); normals.push(nx,ny,nz); texCoords.push(1,0);
      indices.push(3*i, 3*i+1, 3*i+2);
   }
   if (!noBottom) {
      var center = [0, -height/2, 0];
      var idx = vertices.length/3;
      vertices.push(center[0],center[1],center[2]); normals.push(0,-1,0); texCoords.push(0.5,0.5);
      for (var i = 0; i < sides; i++) {
         var p1 = baseVerts[i], p2 = baseVerts[i+1];
         vertices.push(p1[0],p1[1],p1[2]); normals.push(0,-1,0); texCoords.push(0.5+0.5*p1[0]/radius,0.5+0.5*p1[2]/radius);
         vertices.push(p2[0],p2[1],p2[2]); normals.push(0,-1,0); texCoords.push(0.5+0.5*p2[0]/radius,0.5+0.5*p2[2]/radius);
         indices.push(idx, idx+2*i+1, idx+2*i+2);
      }
   }
   return {
